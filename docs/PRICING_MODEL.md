...
| 2026-09-19 | **Link requests, detach and the usage statement built (decision #16, relationship 3) — working tree, not deployed.** `credit_link_requests` table; `create/list/cancel_link_request`, `list_incoming_link_requests`, `approve/decline_link_request`, `unlink_company`, `billing_link_view`, `usage_statement`, routes under `/api/organizations/…`; UI: an inbox + Dashboard notice for the person asked, a "Who pays" panel (detach with confirmation, ask by email, request outcomes) and a Usage statement. **Design choices:** a request is addressed to an EMAIL, never to a specific company — the recipient picks which company they own, so nothing reveals who has an account; the reply is identical either way and mail goes only to existing accounts (no relay for strangers); one open request per (company, email) with caps of 10 open / 20 per day; the URL-match suggestion is deferred to the domain-verification round because it would otherwise leak who uses PaisaMap. **Rules:** only the addressee (matched on the verified Google email) who OWNS the chosen company can approve; the requester must still be an owner/admin of the payer at approval time; a payer can't itself be paid for (no chains) and a company that pays for others can't be paid for; a company holding its own credits can't be linked (they'd be stranded); **a company someone already pays for must be detached first** (found while writing tests: otherwise a second request could silently take a client from its payer, who would never be told); the shared safety rules now live in one place (`_apply_payer`) used by both direct linking and approvals. Detach is immediate from either side, notifies the other side, drops the payer's cap, keeps the company's own personal allowances, moves no credits. The statement is credits spent (never purchases) per company / person / kind of action for a chosen period (IST months or last 30 days) and carries no project, location or wallet-balance data. 149 checks; **42 hand-written mutants, all killed** (one survivor exposed a missing test that ineligible companies are not offered). Browser pass caught a real bug the unit tests couldn't: the Who-pays panel didn't refresh after approving. |
| 2026-09-20 | **Round 4 built — working tree, not deployed: website verification + connect-by-website, company-attributed API keys, and the legacy→v2 tier mapping.** **Website verification:** a company's website is only a claim until proven — a `<meta name="paisamap-site-verification">` tag in the home page's `<head>` (fetched by us, SSRF-hardened: public addresses only, the vetted IP is the one connected to, ports 80/443, ≤3 redirects that must stay on the domain or its `www.` twin, body capped, a tag in `<body>` doesn't count), or a Google Search Console account already connected to the company that is a verified **owner** (`siteOwner`) of the domain. **Deviation from decision #16, on purpose:** GA4 is NOT accepted (nor Search Console "full user") — agencies are routinely given Viewer/Editor/Administrator on a client's property, so it would let an agency "verify" its client's website. One verified company per domain (partial unique index + a check); changing the website to a different domain drops the proof; checks are throttled (10 s per company); `domain_taken` is only ever said to someone who has just proved control, so it can't be used to probe. **Connect-by-website:** a paying company types a website; if a company has verified it AND is findable, the asker learns only yes/no (never a name, owner, email or id — masked in the outgoing list until approved), limited to 30 lookups/day/person; the request goes to every **owner** of that company, only an owner can approve, and only for the company it was addressed to and only while its verification still stands; nothing auto-links. **Judgment call to challenge:** findability is ON by default with a plain-language checkbox next to the verification (the company chose to verify); the alternative is off-by-default. **API keys:** `api_keys.org_id`; a company's owners/admins (and its payer's) list and revoke every key with maker + today's usage; capacity is one bucket per company (`_ops.check`), so extra keys buy no throughput; a key dies when its owner leaves the company (checked on every request AND revoked so re-adding doesn't revive it) or the company is deleted; the lookup falls back to the old query if the column isn't there yet, so a deploy that beats the migration can't lock API customers out. **Legacy→v2 mapping (the open item from #15):** the enforced surface is only two things — the Pro signal columns and the raised API limit — so both plan vocabularies now answer through `_pricing.entitlements()`; v2 dashboard tiers give the Pro columns, and the raised API only where the price-book row has `export_api` (Growth and up). `get_effective_plan_for_user` now honours a company's v2 tier (best of personal and company, a v2 tier winning a tie); `/api/auth/me` keeps `plan` in the old vocabulary for the map and adds `tier`/`tier_label`. `plan_mapping.py` sets ONLY `organizations.plan` (never `users.plan`, so it is inert until `BILLING_SCOPE=wallet` and fully reversible); the default is the **lowest v2 tier that keeps everything the account can do today — Growth for both legacy `pro` and `team`** (`LEGACY_PLAN_FLOOR` said Starter, which would have taken the raised API limit away from the owner's own key); a mapping that would give less is blocked unless `--allow-downgrade`; `--override EMAIL=TIER` for the owner's account (only accounts already on a legacy paid plan — no accidental comps); compare-and-swap writes, an audit row per change, `rollback`. A later legacy purchase never overwrites a company's v2 tier. **Open, not fixed here:** the live checkout still sells legacy Pro at ₹999, which gives the same enforced entitlements as v2 Growth (₹12,000) — cheaper than the tier it is meant to be below; retire or reprice legacy checkout before promoting v2 prices. Seat/company-count limits and monthly credit grants are not enforced anywhere yet (they arrive with subscriptions), so a mapped tier changes only the two entitlements. **Tests:** 55 + 211 + 59 + 73 new checks (both billing scopes), the three older billing suites and isolation/security re-run green; **78 hand-written mutants: 75 killed, 3 equivalent** (loopback/reserved/non-http scheme are each also refused by another check in the same function — layered defences); 9 first-pass survivors were real test gaps (revoke through the wrong company while the key is live, leaving one company killing keys in another, revoked keys counted toward capacity, a changed domain reusing a token, a website edited mid-check, unbounded redirect count, a tag after `<body>` with no `</head>`, `compat_plan` for Scale+, an extra company with its own legacy plan being mapped). Browser-verified (24 checks: pending/verified states, the tag, a failed check's plain-language reason, findability toggle, find → request → owner approves, masked outgoing rows, company key list/revoke, plan labels, no legacy plan cards for a v2 account). **Deploy:** runbook steps 5c (columns first, then the table + key backfill) and 5d (mapping, last). |
